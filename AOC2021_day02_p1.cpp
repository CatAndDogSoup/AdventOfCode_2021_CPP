...
int main()
{
	...
	if (input_file.good()) {} else { std::cout << "Input File not correct!" << std::endl; return 1; }

	// std::vector<std::pair<std::string, unsigned int>> inputs = std::vector<std::pair<std::string, unsigned int>>();
	...
	{
		if (std::get<0>(data) == "up") { depth = depth - std::get<1>(data); }
		else if(std::get<0>(data) == "down") { depth = depth + std::get<1>(data); }
		else if(std::get<0>(data) == "forward") { position = position + std::get<1>(data); }	
	}
